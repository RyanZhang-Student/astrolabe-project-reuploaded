...
def analyze_fifth_house(user_email: str, user_name: str, language: str = 'zh-CN') -> str:
    """
    Reads the 5th House HTML file for the user and generates an AI analysis.
    """
    ...
    # In order to read the house 5 chart result html
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    report_path = os.path.join(base_dir, 'results', user_email, user_name, f'{user_name}_HOUSE 5.html')
    ...
    if os.path.exists(report_path):
        ...
        return f"Error: Could not find House 5 report for user {user_name}. Looked at: {report_path}"

    # Default to English prompt
    prompt = f"""
    You are an expert, premium astrologer interface.
    I will provide you with the HTML structure of a generated Astrolabe report for a user's 5th House.
    Based on the specific data inside this HTML (including Classical Lord Evaluation, Modern Placements, Fixed Star Conjunctions, and Major Aspects), please provide a detailed, deeply insightful, and beautifully written astrological analysis focusing specifically on **House 5 (Creativity, Pleasure, and Romance)**.
    ...
    if language == 'zh':
        prompt = f"""
        你是一个专家级的高端占星分析引擎。
        我将为你提供一个用户 Astrolabe 星盘报告的 HTML 结构，专门针对第 5 宫。
        基于这个 HTML 内部的具体数据（包括古典占星守护星评估、现代落位、恒星合相和主要相位），请提供一份详细、见解深刻且文笔优美的占星分析，专门针对 **第 5 宫**。
    ...
    if language == 'en':
        ...
        - Concept Introduction: Explains the core definition and practical significance of this house in astrology or in the relevant area of life.
        - Foundation & Blueprint: Analyzes the zodiac sign ruling the cusp of this house, interpreting the innate traits, image, and potential challenges it brings to that area of life.
        - Core Drivers: Ruler Alignment: Identifies the zodiac sign, house, and retrograde status of the ruling planet (house ruler) for this house, and provides a detailed breakdown of the network of aspects form between this ruling planet and the Sun, other core celestial bodies, and virtual points.
        - Energy Matrix: Planetary Distribution and Aspects: Analyzes the planets and asteroids directly located within the house, the energy at the cusp, and their interactive aspects with other celestial bodies.
        - Deep Insights: Hidden Influences: Explore conjunctions with fixed stars within the house or near the ruling planet to reveal more hidden, deeper destinies or special empowerments.
        - Dynamic Analysis: Organically integrate all the above elements (zodiac signs, ruling planet, inner planets, fixed stars) to distill an overall energetic picture of this domain.
        - House Qualitative Summary: Provides a concise, final qualitative assessment of the house’s core energy, strengths, and risks.
    ...
    if language == "hi":
        prompt = f"""
        तुम एक विशेषज्ञ ज्योतिषी हो जो जन्म चार्ट के भावों का गहन विश्लेषण कर रहा है।
        जन्म चार्ट के 5 भाव का विश्लेषण करें।
        
        कृपया अपने उत्तर की शुरुआत एक मुख्य शीर्षक से करें:
        ## भाव 5 का गहन विश्लेषण
        
        इन खंडों को शामिल करें:
        - अवधारणा का परिचय: इस भाव का अर्थ।
        - नींव और ब्लूप्रिंट: राशि चक्र और उसकी ऊर्जा।
        - मुख्य चालक: सत्तारूढ़ ग्रह और पहलू।
        - ऊर्जा मैट्रिक्स: अन्य ग्रहों के साथ बातचीत।
        - गहरी अंतर्दृष्टि: निश्चित तारे (Fixed Stars)।
        - गतिशील विश्लेषण: समग्र चित्र।
        - भाव का गुणात्मक सारांश: ताकत और जोखिम।
        - मुख्य कार्रवाई रणनीति: 8-10 सिफारिशें।
        
        दिशा-निर्देश:
        - महत्वपूर्ण: केवल हिंदी (Hindi) में लिखें।
        - सुंदर मार्कडाउन का उपयोग करें।
        - ज्यामितीय डिग्री (जैसे 12.34°) शामिल न करें।
    ...
    try:
        ...
        return f"An error occurred during Gemini API call for House 5: {str(e)}"
