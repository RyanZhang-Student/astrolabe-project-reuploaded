...
def analyze_eleventh_house(user_email: str, user_name: str, language: str = 'zh-CN') -> str:
    """
    Reads the 11th House HTML file for the user and generates an AI analysis.
    """
    ...
    # In order to read the house 11 chart result html
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    report_path = os.path.join(base_dir, 'results', user_email, user_name, f'{user_name}_HOUSE 11.html')
    ...
    if os.path.exists(report_path):
        ...
        return f"Error: Could not find House 11 report for user {user_name}. Looked at: {report_path}"

    # Default to English prompt
    prompt = f"""
    You are an expert, premium astrologer interface.
    I will provide you with the HTML structure of a generated Astrolabe report for a user's 11th House.
    Based on the specific data inside this HTML (including Classical Lord Evaluation, Modern Placements, Fixed Star Conjunctions, and Major Aspects), please provide a detailed, deeply insightful, and beautifully written astrological analysis focusing specifically on **House 11 (Friends, Groups, and Wishes)**.
    ...
    if language == 'zh':
        prompt = f"""
        你是一个专家级的高端占星分析引擎。
        我将为你提供一个用户 Astrolabe 星盘报告的 HTML 结构，专门针对第 11 宫。
        基于这个 HTML 内部的具体数据（包括古典占星守护星评估、现代落位、恒星合相和主要相位），请提供一份详细、见解深刻且文笔优美的占星分析，专门针对 **第 11 宫**。
    ...
    if language == "hi":
        ...
        जन्म चार्ट के 11 भाव का विश्लेषण करें।
        
        कृपया अपने उत्तर की शुरुआत एक मुख्य शीर्षक से करें:
        ## भाव 11 का गहन विश्लेषण
    ...
    try:
        ...
        return f"An error occurred during Gemini API call for House 11: {str(e)}"
