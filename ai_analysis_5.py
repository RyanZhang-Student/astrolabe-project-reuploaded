...
# API Key 配置
GEMINI_API_KEY = "AIzaSyAsG8CMK-9Z2wDCWkmDCgvjN67aw5oXwuA"

def generate_and_append_analysis_5(html_file_path):
    ...
    try:
        # 简化的 Prompt 逻辑：移除了原有的 Prompt Intro
        # 注意：这里仅发送 HTML 内容，Gemini 会根据内容自行总结或分析。
        # 如果需要特定分析（如第2宫），请在此处添加 prompt_intro 文本。
        prompt_intro = """
        你是一个专业的占星师。请仔细阅读以下占星报告 HTML 代码中的数据表格，提取关于**第5宫的所有相关信息。
        
        请严格按照以下模板的格式和分点结构，为我生成第5宫的深度分析报告 .
        ...
        <details style="max-width: 800px; margin: 40px auto; padding: 20px; background-color: #f8f9fa; border-left: 5px solid #4B0082; border-radius: 8px;">
            <summary style="cursor: pointer; font-weight: bold;">
                <h2 style="color: #333; display: inline;">🔮 深度分析：第5宫</h2>
            </summary>
        ...
        # 在 </body> 前插入，这样会排在 ai_analysis_1 的块之后（如果 ai_analysis_1 已经运行过）
